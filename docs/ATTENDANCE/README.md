...
- 先刷工牌或先人脸都可配对，配对后的有效时间可在后台选择：默认取两条记录中较晚的一条，也可改为取较早的一条。
- 飞书点位命中“免工牌点位前缀”时，该人脸打卡直接生成有效考勤，不再要求 5 分钟内有本地工牌刷卡。
- 飞书考勤组命中“直记考勤组”时，该组任意飞书流水直接生成有效考勤，不再要求双验证，也不发送有效考勤成功提醒。
- 飞书流水 `type` 命中“直记Type”时，对所有考勤组生效，直接生成有效考勤；未命中的考勤组仍按双验证规则计算。
...
| 飞书假勤流水 | `attendance_source_records` | 通过事件订阅实时写入，也可通过全量任务批量拉取；`flow_type` 保存飞书流水 `type`，飞书历史 `工牌-` 流水可补齐本地缺失的工牌证据。 |
...
飞书流水 `type` 的含义按飞书假勤返回字段配置：`0` 用户自己打卡、`1` 管理员修改、`2` 用户补卡、`3` 系统自动生成、`4` 下班免打卡、`5` 考勤机打卡、`6` 极速打卡、`7` 开放平台导入。

## 配对算法

1. 按人员和自然日读取源流水，按 `punch_time ASC, id ASC` 排序。
2. 命中“直记考勤组”的飞书流水直接生成有效记录，优先级最高。
3. 命中“直记Type”的飞书流水直接生成有效记录，对全部考勤组生效。
4. `exempt_face` 或命中免工牌点位前缀的飞书流水直接生成有效记录。
5. `feishu_badge` 如果找到同时间本地 `badge`，只作为对照；如果本地缺失，则按工牌记录参与配对。
6. `badge` / `feishu_badge` 与 `face` 双队列互相匹配，选择配置间隔内时间差最小的未使用记录。
7. 一条源流水只允许被消费一次。
8. 双认证有效时间按 `attendance_pair_effective_time_rule` 决定：`latest` 取 `max(badge_time, face_time)`，`earliest` 取 `min(badge_time, face_time)`。
9. 生成的有效考勤按有效时间排序并写入日报。

示例：`工牌 人脸 工牌 人脸` 生成 2 次有效考勤；`工牌 人脸 工牌 工牌 工牌 人脸 人脸 工牌` 会按 5 分钟窗口和最近未使用记录配对，避免同一条人脸或工牌被重复消费。

切换有效时间规则后不会自动重算已生成的有效考勤和日报；后续新产生的考勤计算会立即使用新规则。管理员手动指定历史日期重算时，仍按重算时的当前设置执行。
...
提醒只在单人单日增量重算生成双验证有效考勤时入队；全量同步、批量历史重算、直记考勤组、直记Type、免工牌点位不会主动推送成功提醒，避免集中补算或飞书原生考勤组打扰员工。
...
同一人员、同一上班/下班阶段、同一方式在配对窗口内连续产生多条单边流水时，只对第一条发送补刷提醒。例如先刷脸进入 5 分钟配对窗口后继续连续刷脸，后续刷脸不会重复提醒。

命中直记考勤组、直记Type、免工牌点位的流水不进入补刷提醒。
